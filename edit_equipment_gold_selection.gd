...
func _process(delta: float) -> void:
	$Label.text = "Equipment: " + str(Global.gold) + " Gold"
...
func _ready():
	for item_name in Global.fort_equip.keys():
		...
		if item_cost <= Global.gold:
			create_fort_item_button(item_name, item_slot, item_desc, item_cost, item_data)
		elif item_name in Global.equip_purchased:
			# Recreate the button for the already purchased item
...
func update_equip_stats():
	...
	for item_data in Global.equip_purchased:
		if "fortitude_attrition" in item_data and item_data["slot"] != "Main Hand" and item_data["slot"] != "Both Hands" and item_data["slot"] != "Off-Hand":
			Global.equip_bonus_fort_att += item_data["fortitude_attrition"]  # Add the resistance value to the total
	for item_data in Global.equip_purchased:
		if "finesse_attrition" in item_data and item_data["slot"] != "Main Hand" and item_data["slot"] != "Both Hands" and item_data["slot"] != "Off-Hand":
			Global.equip_bonus_fin_att += item_data["finesse_attrition"]  # Add the resistance value to the total
	for item_data in Global.equip_purchased:
		if "focus_attrition" in item_data and item_data["slot"] != "Main Hand" and item_data["slot"] != "Both Hands" and item_data["slot"] != "Off-Hand":
...
func _on_next_button_pressed() -> void:
	update_equip_stats() # Replace with function body.
...
func _on_check_button_toggled(toggled_on: bool) -> void:
	if $Label2/CheckButton.button_pressed == false:
		get_tree().change_scene_to_file("res://edit_equipment_tab.tscn")
		$Label2/CheckButton.button_pressed == true
	else:
		pass
